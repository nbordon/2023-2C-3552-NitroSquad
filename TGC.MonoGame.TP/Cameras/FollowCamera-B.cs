...
namespace TGC.MonoGame.TP.Cameras
{
    ...
    class FollowCamera
    {
        ...
        private Vector3 CurrentFrontVector { get; set; } = Vector3.Forward;

        private float FrontVectorInterpolator { get; set; } = 0f;

        private Vector3 PastFrontVector { get; set; } = Vector3.Forward;
        ...
        public void Update(GameTime gameTime, Matrix followedWorld)
        {
            ...
            if (Vector3.Dot(followedFront, PastFrontVector) > AngleThreshold)
            {
                ...
                FrontVectorInterpolator = MathF.Min(FrontVectorInterpolator, 1f);
                ...
                CurrentFrontVector = Vector3.Lerp(CurrentFrontVector, followedFront, FrontVectorInterpolator * FrontVectorInterpolator);
            }
            else
                // Si el angulo no pasa de cierto limite, lo pongo de nuevo en cero
                FrontVectorInterpolator = 0f;

            // Guardo el vector Derecha para usar en la siguiente iteracion
            PastFrontVector = followedFront;
            ...
            var Front = Vector3.Cross(forward, Vector3.Up);
